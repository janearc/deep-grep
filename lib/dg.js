...
exports.deeply = function ( deep, test ) {
	...
	//
	// So the original effort here is to assign deep to S.deep (the present
	// argument to the singleton), and then just keep runningn the examine
	// subroutine until S.deep is exhausted (which happens in examine()). At
	// which point, S.results is returned to the caller. The 'return value' of
	...
	for (; function (t) {
		if (S.deep.length > 0) {
			...
			if ( Object.prototype.toString.call( element ).substr(8, 5) === 'Array' ) {
				element.forEach( function (subelement) { S.deep.push( subelement ) } );
				return true;
			}
